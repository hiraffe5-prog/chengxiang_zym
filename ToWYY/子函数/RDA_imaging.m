function [s_ac,R0_RCMC] = BPA_imaging(Para,raw_data_type)
%% 载入参数
fs = Para.fs;
Naz = Para.Naz;
Nr = Para.Nr;
...
if raw_data_type == 1
    ...
    pos_xyz = Para.pos_xyz(:,1:Naz-B);
    pos_fit = round((Naz-B)/2);
end
if raw_data_type == 2
...
t_ref_mtx = ones(Naz-B,1)*t_ref;      % 矩阵形式
...
% s_ref = [s_ref(:,1:Nr/2),zeros(Naz-B,Nrg-Nr-1),s_ref(:,Nr/2+1:end)];
% ************************************************************************
...
Vx0 = 3*px(1)*st(1,pos_fit)^2 + 2*px(2)*st(1,pos_fit) + px(3); %三维速度
Vy0 = 3*py(1)*st(1,pos_fit)^2 + 2*py(2)*st(1,pos_fit) + py(3);
Vz0 = 3*pz(1)*st(1,pos_fit)^2 + 2*pz(2)*st(1,pos_fit) + pz(3);
    
velocity_tar = [Vx0,Vy0,Vz0];

R_vector_ref = [(pos_xyz(1,pos_fit) ).',...
            (pos_xyz(2,pos_fit) ).',(pos_xyz(3,pos_fit) ).'];
Vm_ref= dot(R_vector_ref, velocity_tar) / norm(R_vector_ref);
fnc = - 2*Vm_ref/lamda;

Vx = 3*px(1)*st(1,:).^2 + 2*px(2)*st(1,:) + px(3); %三维速度
Vy = 3*py(1)*st(1,:).^2 + 2*py(2)*st(1,:) + py(3);
Vz = 3*pz(1)*st(1,:).^2 + 2*pz(2)*st(1,:) + pz(3);
...
S_2df = fft(s_rc,1);    	% 方位向傅里叶变换，到距离多普勒域。
S_2df = fft(S_2df,2);   	% 距离向傅里叶变换，到二维频域
% ！！！注意：距离向零频在两端。！！！
% ====================================================================
% 设置方位频率轴——这是关键点
fa = fnc + fftshift(-Na/2:Na/2-1)/Na*Fa; 	% 方位频率轴如此设置。
% =====================================================================
D_fn_Vr = sqrt(1-lamda^2.*(fa.').^2./(4*Vr^2));         % 大斜视角下的徙动因子
K_src = 2*Vr^2*f0^3.*D_fn_Vr.^3./(c*R0*(fa.').^2);      % 列向量Z
K_src_1 = 1./K_src;     	% 列向量。为了后面能使用矩阵乘法，这里先求倒数，1/Z，SRC滤波器
