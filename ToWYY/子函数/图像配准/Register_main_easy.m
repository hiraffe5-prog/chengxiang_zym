% 本代码的作用是利用之前生成的配准参数直接进行图像配准
function [slave_reg]=Register_main_easy(master,slave,search_row_col,sample_block_length,search_block_length,threshold_cor,overlapping,upsample_row_col)
%% 
% 第一步：校正图像整体像素偏移
% 第二步：校正图像像素级几何畸变
% 第三步：校正图像亚像素级几何积畸变
...
%     [delta_row,delta_col] = Phase_cor(master_temp,slave_temp,Na+search_row,Nr+search_col,sample_block_row,sample_block_col,search_block_row,search_block_col);
    %%
    load SaveData/Phase_cor.mat delta_row delta_col
    if delta_row>0
...
%     %}
    % 对配准像素进行二维拟合
    load SaveData/Match_subpix.mat row_pix_master column_pix_master row_offset col_offset max_cor
