...
%————————————————————————————————————————————————%
function [slave_reg]=Register_main_easy_v3(master,slave,search_row_col,sample_block_length,search_block_length,threshold_cor,overlapping,upsample_row_col,flag_pl,flag_spl,No)
%% 
% 第一步：校正图像整体像素偏移
% 第二步：校正图像像素级几何畸变
% 第三步：校正图像亚像素级几何积畸变
...
%     slave_temp(search_row+1:Na+search_row,search_col+1:Nr+search_col)=slave;
 ...
 if (flag_spl == 1)                                                                                     % 需要进行精配准的情况
    ...
    %}
    % 对配准像素进行二维拟合
    load SaveData/Match_subpix_0.mat row_pix_master column_pix_master row_offset col_offset max_cor
    ...
    fitobjectx=fit([x,y],z_row,'poly33');             % 三阶拟合方程
    z_column=P_new;
    %拟合方程
    % fitobjecty=fit([x,y],z_column,'poly22');  % 二阶拟合方程
    fitobjecty=fit([x,y],z_column,'poly33');        % 三阶拟合方程
    % 对图像进行重采样
    xx=zeros(Na,Nr);
    yy=zeros(Na,Nr);
    for ii=1:Na
